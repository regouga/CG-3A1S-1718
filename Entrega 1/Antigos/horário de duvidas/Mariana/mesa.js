...
function createTrack(mesh1) {
  ...
  for(z = -80; z < 50; z+= 10) {
    ...
    for(teta=-Math.PI/2; teta>-Math.PI; teta-=Math.PI/22) {
    ...
    for(teta=-Math.PI; teta>0; teta-=Math.PI/22) {
      createCheerio(mesh1, 140 * Math.cos(teta), 52.5,  140 *Math.sin(teta));
    }
}
