...
var tableMaterialsLambert;
var tableMaterialsPhong;
var tableMaterialsBasic;
...
var spriteImg = 'textures/car.png';
var restartScreen = 'textures/restart.png';
	...
	changeMaterials(flagLetra) {
		if (flagLetra == 0){ //significa que foi ativado o G
			...
			if (this.basicMaterial == 0){
				...
				this.children[1].material = butterMaterialsBasic[1];
			}
			else{
				this.basicMaterial = 0;
				if (this.lightmaterial == 0) { //se tivermos um shadding phong
...
class Mesa extends Object3D {
	constructor(x, y, z) {
		super();
		var texture = new THREE.TextureLoader().load("textures/towel.jpg");
		material = new THREE.MeshPhongMaterial({color: 0xA9A9A9, wireframe: false /*, map: texture*/});
	...
	addTableTop(x, y, z, mate) {
		'use strict';
		geometry = new THREE.CubeGeometry(700, 100, 400, 2,2 );
...
function lifeFunction(){
	if(lifes > 1){
		lifes -= 1;
		spritegroup.children[lifes].visible = false; 		
	}
	else{
		lifes = MaxLifes;
		spritegroup.children[0].visible = false;
...
//LARANJAS
	...
	orangeMaterialsBasic.push(materialLaranja2);
...
//CARRO
	...
	material = new THREE.MeshPhongMaterial({color: 0xbf005f, wireframe: true});
	carMaterialsPhong.push(material);
	materialArray.push(material);

	material = new THREE.MeshLambertMaterial({color: 0xbf005f, wireframe: true});
	carMaterialsLambert.push(material);
	materialArray.push(material);

	material = new THREE.MeshBasicMaterial({color: 0xbf005f, wireframe: true});
...
//CANDLE
	...
	candleMaterialsPhong.push(materialLaranja);
	candleMaterialsPhong.push(materialLaranja2);
...
material = new THREE.MeshPhongMaterial({color: 0xA9A9A9, wireframe: false}); 
//material = new THREE.MeshPhongMaterial({color: 0xA9A9A9, wireframe: true/*, map: texture*/});
tableMaterialsPhong = material;
materialArray.push(material);

material = new THREE.MeshLambertMaterial({color: 0xA9A9A9, wireframe: true/*, map: texture*/});
tableMaterialsLambert = material;
materialArray.push(material);

material = new THREE.MeshBasicMaterial({color:0xA9A9A9, wireframe: true/*, map: texture*/});
tableMaterialsBasic = material;
materialArray.push(material);
...
function createAll() {
	...
	car = new Car(0, 53, 155, carMaterialsPhong[0], carMaterialsPhong[1], carMaterialsPhong[0], carMaterialsPhong[1], carMaterialsPhong[2]);
...
//RODA ESQUERDA DE TRAS
	createFace(faces, 16, 24, 20, 28, 1); //parte roda tras
	createFace(faces, 24, 25, 28, 29, 1); // parte roda cima
	createFace(faces, 25, 29, 17, 21, 0); //parte roda frente
	createFace(faces, 17, 19, 21, 23, 0); //parte roda reta frente
...
function createSpotlight() {
	spotLight1 = new THREE.SpotLight( 0xffffff,5, 1000, Math.PI, 1);
	spotLight1.rotateZ(Math.PI /4);
	spotLight1.angle = Math.PI / 5;
	spotLight1.distance = 250;
	spotLight1.target.position.set(30,0,0);
	car.add(spotLight1);
	spotLight1.position.set(0, 0, -4);
	car.add(spotLight1.target);

	spotLight2 = new THREE.SpotLight( 0xffffff,5, 1000, Math.PI, 1);
	spotLight2.rotateZ(Math.PI /4);
	spotLight2.angle = Math.PI / 5;
	spotLight2.distance = 250;
	spotLight2.target.position.set(30,0,0);
	car.add(spotLight2);
...
function createCarImage(x, y, z) {
	var geometry = new THREE.PlaneGeometry(6, 3, 0);

	var texture = new THREE.TextureLoader().load( spriteImg );
...
function createRestartScreen() {
	...
	var texture = new THREE.TextureLoader().load(restartScreen);

    var material = new THREE.MeshBasicMaterial( { map: texture } );

    restart = new THREE.Mesh(geometry, material);
...
function addHUD() {
	...
	createCarImage(-35, 20, 0);
	createCarImage(-32, 20, 0);
	createCarImage(-29, 20, 0);
	createCarImage(-26, 20, 0);
	createCarImage(-23, 20, 0);
...
function checkMove(){										// Detecao
	...
	if (car.position.x > 350 || car.position.x < -350 || car.position.z > 200 || car.position.z < -200){
		lifeFunction();
		car.position.set(0, 53, 155);
...
function restartGame(health) {
	var removing = objectsgroup.children.length - 1;
	
	while (removing > 0) {
		objectsgroup.remove(objectsgroup.children[removing]);
		removing--;
	}
...
function reAddOrange(laranjinha) {
	if (!pauseFlag) laranjinha.visible = true;
}
