...
function resizeOrtCamera(scale) {
	cameraArray[0].left = -window.innerWidth / scale;
	cameraArray[0].right = window.innerWidth / scale;
	cameraArray[0].top = -window.innerHeight / scale;
	cameraArray[0].bottom = window.innerHeight / scale;
	cameraArray[0].updateProjectionMatrix();
}
...
function moveLEFTL(object) {										// Esquerda
	'use strict';
...
function moveRIGHTL(object, delta) {										// Direita
	'use strict';
	rotate(car, new THREE.Vector3(0,1,0), -(Math.PI/48) + delta);
