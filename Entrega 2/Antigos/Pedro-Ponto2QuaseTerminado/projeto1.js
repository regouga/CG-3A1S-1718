...
function createOrange(materialOrange, x, y, z, angulo) {
	'use strict';
...
function createScene() {
	...
	//gerar as laranjas em posicoes pseudo aleatorias

	while (numeroOranges < 3) {
		var Ox = Math.random() * (maxX - minX) + minX;
		var Oy = Math.random() * (maxY - minY) + minY;
		var ANGLE = Math.random() * (MAX_ANGLE - MIN_ANGLE) + MIN_ANGLE;
		createOrange(material, Ox, 56, Oy, ANGLE);
		numeroOranges++;
	}
...
function removeOrange() {
	for (var i in arrayOranges) {
		if (arrayOranges[i].position.x > 350 || arrayOranges[i].position.x < -350 || arrayOranges[i].position.z > 200 || arrayOranges[i].position.z < -200) {
			...
			arrayOranges[i].position.set(Ox, 56, Oy);
